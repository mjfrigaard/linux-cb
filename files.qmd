...
*After copying `.csv` files into `data/csv/`:*[^tree-pattern-csv]

:::

[^tree-pattern-csv]: The `-L 2` option tells tree to only look in the data folder (no subfolders) and `-P '*.csv'` matches the `.csv` files.
...
Oops--it looks like the `who_tb_data.psv` file was created in the `data` folder (and not the `data/psv` folder): [^tree-pattern-or]
...
[^tree-pattern-or]: The `-L 2` option tells tree to only look in the data folder (no subfolders) and `-P '*who_tb_data.psv|*who_tb_data.csv'` matches the `who_tb_data.psv` *or* the `who_tb_data.csv` file.
