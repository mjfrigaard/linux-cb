...
Plain text files are vital to Unix/Linux systems, embodying their philosophy of 'simple and beautiful.'[^mcilroy] These files only contain text, making them versatile and powerful. Unix/Linux believes that '*everything is a file*,' including devices, configurations, and processes.[^file-vs-bits] Plain text files are the universal interface between systems, programs, and users. Standard Unix/Linux tools can easily create, manipulate, and read plain text, making it an essential interface for system administration, programming, and process management.

[^mcilroy]: Doug McIlroy on Unix programming: "*Write programs that do one thing and do it well. Write programs to work together. Write programs to handle text streams, because that is a universal interface.*" - [Wikipedia](https://en.wikipedia.org/wiki/Unix_philosophy#Doug_McIlroy_on_Unix_programming)
