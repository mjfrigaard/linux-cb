...
[**Bash, or the Bourne Again SHell**](https://www.gnu.org/software/bash/), is one of the most widely used shells in Unix and Linux environments. Incorporating features from the Korn shell ([`ksh`](https://en.wikipedia.org/wiki/KornShell)) and the C shell ([`csh`](https://en.wikipedia.org/wiki/C_shell)), Bash supports features like command history, tab completion, aliases and scripting tasks.[^bash-macos] 

[^bash-macos]: Since the release of macOS Catalina (2019), [Apple transitioned](https://support.apple.com/en-us/102360) from using Bash as the default command-line interface to Zsh.
